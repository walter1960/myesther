...
function loadHistoryCards() {
    ...
    container.innerHTML = '<div class="w-full text-[10px] font-black text-gray-400 tracking-widest uppercase mb-2 mt-2 px-1">Salons Récents :</div>' + history.map(s => {
        ...
        return `
        <div class="flex items-center justify-between bg-white border border-gray-100 p-2.5 rounded-2xl w-full mb-2 hover:border-brand hover:shadow-sm transition-all group">
            <div class="flex-1 cursor-pointer" onclick="document.getElementById('secret-input').value='${s.secret}'; document.getElementById('alias-input').value='${s.alias}'; establishSecureTunnel();">
                <div class="font-black text-sm text-gray-800 tracking-tight">${s.groupName}</div>
                <div class="flex items-center gap-2 mt-1">
                    <span class="text-[9px] font-bold text-gray-400 uppercase tracking-widest">${s.alias}</span>
                    ${timeBadge}
                </div>
            </div>
            <button class="w-8 h-8 flex items-center justify-center text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-colors ml-2" onclick="deleteHistoryItem('${s.secret}')" title="Détruire le salon">
